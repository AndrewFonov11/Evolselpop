			...
			if ((rs[i] > 4) && (rnd == 1)) {
				for (j = 0; j < 450; j++) {
					if (ppl[j] == 0) {
						...
						if (rnd == 1) gen[j]++;
						if (rnd == 2) gen[j]--;
						if ((rnd == 3) && (sn == 2)) gen[j]+=2;
						if ((rnd == 4) && (sn == 2)) gen[j]-=2;
						if ((gen[j] > 15) && (sn != 1)) rs[j] += gen[j]-15;
						if ((gen[j] > 15) && (sn == 1)) rs[j] += 1;
						if ((gen[j] > 20) && (sn == 1)) rs[j] += gen[j]-20;
						j=450;
					}
				}
			}
	...
	for (i = 0; i < 450; i++) {
		for (j = 0; j < 655; j++) {
			if ((ppl[i] == 1) && (ppl_y[i] == by[j]) && (ppl_x[i] == bx[j])) {
				...
				by[j] = rand() % 32;
				if (sn == 0) rs[i] += gen[i];
				if (sn == 1) rs[i] += gen[i]-12;
				if (sn == 2) rs[i] += gen[i]+4;
			}
		}
		if (gm[ppl_y[i]][ppl_x[i]] == '=') {
			gm[ppl_y[i]][ppl_x[i]] = ' ';
			if (sn == 0) rs[i] -= gen[i]-16;
			if (sn == 1) rs[i] -= gen[i]-18;
			if (sn == 2) rs[i] -= gen[i]-10;
		}
	}
}
